...
template <typename TChar, typename TAllocConfig, typename TIdx>
TIdx loadIndex(CharString const indexPath)
{    
    typedef String<TChar, TAllocConfig> TString;
    typedef Index<StringSet<TString, Owner<ConcatDirect<> > >, TIndexConfig> TIdx;
    TIdx index;
...
template <typename TChar, typename TIdx>
TIdx loadIndex(CharString const indexPath, bool const mmap)
{
    if(mmap)
        return(loadIndex<TChar, MMap<> >(indexPath));
    else
        return(loadIndex<TChar, Alloc<> >(indexPath));
}
