...
template <typename TText, typename TConfig, typename TIndexSpec,
          typename TDir,
          size_t nbrBlocks>
Pair<uint8_t, Pair<uint32_t, uint32_t>> uniget_bitvector_interval_inside(Iter<Index<TText, FMIndex<void, TConfig> >, VSTree<TopDown<TIndexSpec> > > iter,
...
ReturnCode uniCheckMappability(TDelegate & delegate,
                                 ...
                                 TDir const & )
{
    //TODO start full check on Mappability here also use filter
    Pair<uint8_t, Pair<uint32_t, uint32_t>> bit_interval = get_bitvector_interval_inside(iter, bitvectors, s, blockIndex + 1, TDir());
        ...
        if(rcode == ReturnCode::NOMAPPABILITY){
            cout << "This is very unlikly depending on parameters ... stop" << endl;
...
//     cout << "Reached End" << endl;
    ...
    if(needleRightPos - needleLeftPos - 1 == s.blocklength[blockIndex] && blockIndex > 0){
        bool end = (minErrorsLeftInBlock == 0 && needleLeftPos == 0 && needleRightPos == length(needle) + 1);
        ReturnCode rcode = uniCheckMappability(delegate, delegateDirect, iter, needle, bitvectors, needleLeftPos, needleRightPos, errors, s, blockIndex, end, TDir());
